...
public class Client  extends AnAbstractSimulationParametersBean {
	...
	private static GIPCClientHandler gipcClient;
	...
	public void simulationCommand(String cmd) {
		...
		//gipc
		gipcClient.propertyChange(new PropertyChangeEvent(this, "InputString", null, cmd));
	}
	...
	public static void buildGIPC(String[] args)  {
		...
		gipcClient = new GIPCClientHandler(serverProxy, name);
		gipcRegistry.rebind(gipcClient.getName(), gipcClient.getProxy());
		try {
			serverProxy.join(gipcClient.getName(), gipcClient.getProxy());
		} catch (RemoteException e) {
				e.printStackTrace();
		}
